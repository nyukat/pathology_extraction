...
def get_entity_spans(spans: List[Tuple[int,int]], labels: List[str]) -> List[Dict]:
    """
...
def ner_inference(
    ...
    entity_list = []
    for spans, sent, batch in zip(span_lists, sentences, input_dataset):
        ...
        labels = output.argmax(axis=-1).detach().tolist()
